...
{
	// Node for AStar, stores all values and integrates with priority queue implementation MINUS closed
	internal class GRAStarNodeNoClosed : FastPriorityQueueNode
	{
		...
		public GRAStarNodeNoClosed Parent;
		// (Known) distance from start to this node, by shortest known path

		public GRAStarNodeNoClosed(Coord position, GRAStarNodeNoClosed parent = null)
		{
			Parent = parent;
			Position = position;
			F = G = float.MaxValue;
	...
	// GoRogue AStar, Heuristic/Weight added, using nudging Tiebreaker instead of direction ordering, optimized for dynamic allocation of nodes, using HashSet for closed set.
	public class GR_H_W_T_D_Hash
	{
		// Used to calculate neighbors of a given cell
		private GRAStarNodeNoClosed[] nodes;
		...
		private FastPriorityQueue<GRAStarNodeNoClosed> openNodes;

		private HashSet<Coord> closed;
		...
		/// </param>
		public GR_H_W_T_D_Hash(IMapView<bool> walkabilityMap, Distance distanceMeasurement, Func<Coord, Coord, double> heuristic = null, IMapView<double> weights = null)
		{
			...
			nodes = new GRAStarNodeNoClosed[maxSize];
			closed = new HashSet<Coord>();
			nodesWidth = walkabilityMap.Width;
			nodesHeight = walkabilityMap.Height;

			openNodes = new FastPriorityQueue<GRAStarNodeNoClosed>(maxSize);
		}
		...
		public InternalPath ShortestPath(Coord start, Coord end, bool assumeEndpointsWalkable = true)
		{
			...
			if (nodesWidth != WalkabilityMap.Width || nodesHeight != WalkabilityMap.Height)
			{
				...
				nodes = new GRAStarNodeNoClosed[length];
				openNodes = new FastPriorityQueue<GRAStarNodeNoClosed>(length);
			...
			if (nodes[index] == null)
				nodes[index] = new GRAStarNodeNoClosed(start, null);
			...
			while (openNodes.Count != 0)
			{
				...
				foreach (var dir in ((AdjacencyRule)DistanceMeasurement).DirectionsOfNeighbors())
				{
					...
					if (neighbor == null) // Can't be closed because never visited
						nodes[neighborIndex] = neighbor = new GRAStarNodeNoClosed(neighborPos, null);
					else if (closed.Contains(neighborPos)) // This neighbor has already been evaluated at shortest possible path, don't re-add
						continue;
		...
		private static bool IsOpen(GRAStarNodeNoClosed node, FastPriorityQueue<GRAStarNodeNoClosed> openSet)
		{
