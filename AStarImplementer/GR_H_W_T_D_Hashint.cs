...
{
	// GoRogue AStar, Heuristic/Weight added, using nudging Tiebreaker instead of direction ordering, optimized for dynamic allocation of nodes, using HashSet for closed set that hashes on index of coord.
	public class GR_H_W_T_D_HashInt
	{
		...
		/// </param>
		public GR_H_W_T_D_HashInt(IMapView<bool> walkabilityMap, Distance distanceMeasurement, Func<Coord, Coord, double> heuristic = null, IMapView<double> weights = null)
		{
		...
		public InternalPath ShortestPath(Coord start, Coord end, bool assumeEndpointsWalkable = true)
		{
			...
			while (openNodes.Count != 0)
			{
				...
				closed.Add(current.Position.ToIndex(WalkabilityMap.Width));
				...
				foreach (var dir in ((AdjacencyRule)DistanceMeasurement).DirectionsOfNeighbors())
				{
					...
					else if (closed.Contains(neighborIndex)) // This neighbor has already been evaluated at shortest possible path, don't re-add
						continue;
